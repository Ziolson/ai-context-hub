---
name: tdd-workflow
description: >
  Test-Driven Development workflow for implementing features. Activate
  when the user is ready to write code — guides through the red-green-refactor
  cycle with structured test planning.
---

# TDD Workflow

Read and follow the instructions from the following files:
- `docs/workflows/test-driven-development.md` — Full TDD workflow guide

Also apply the global rules from `rules/global-rules.md`.
